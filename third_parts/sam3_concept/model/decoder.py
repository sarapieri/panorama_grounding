...
import numpy as np
import torch
from third_parts.sam3_concept.sam.transformer import RoPEAttention
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn(self, tgt):
        with torch.amp.autocast(device_type="cuda", enabled=False):
            # Upstream runs the FFN in fp32 assuming fp32 weights; the detector may be bf16 under
            # DeepSpeed while tgt is fp32, so the input is matched to the weight dtype.
            tgt2 = self.linear2(self.dropout3(self.activation(
                self.linear1(tgt.to(self.linear1.weight.dtype)))))
        tgt = tgt + self.dropout4(tgt2.to(tgt.dtype))
        tgt = self.norm3(tgt)
        return tgt
