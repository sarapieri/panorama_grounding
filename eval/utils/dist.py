# Distributed helpers for the eval scripts.
import functools
import os
...
def _init_dist_pytorch(backend, **kwargs) -> None:
    ...
    Args:
        backend (str): Backend of torch.distributed ('nccl', 'gloo' or 'mpi').
...
def _init_dist_slurm(backend, port=None, **kwargs) -> None:
    ...
    # SLURM_LOCALID may be unset with some launchers; fall back to proc_id % num_gpus
...
def collect_results_cpu(result_part: list,
    ...
    Args:
        ...
        size (int): Size of the results, commonly equal to length of
            the results.
        tmpdir (str): Shared directory where the per-rank parts are written.
            Defaults to './dist_test_temp'.
